...
var positions = {'left': Vector2(-128,0), 'right': Vector2(+128,0), 'center': Vector2(0,0)}
var direction = 'left'
var debug = false
...
func fade_in(node = self, time = 0.0):
	...
	if time > 0.0:
		...
		tween_node.interpolate_property(
			node, "rect_position", node.rect_position - end_pos, node.rect_position, time,
