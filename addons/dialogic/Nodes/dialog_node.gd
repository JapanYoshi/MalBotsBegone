...
## I want to add the beepboop thing - Haley
var characters_tweening: bool = false
...
## cache Rects for custom ready indicator
var rect_ready: Rect2
var rect_notready: Rect2
...
func _ready():
	...
	## get custom rects
	rect_ready = Rect2(0, 0, 48, 48)
	rect_notready = rect_ready
	rect_ready.position.x += rect_notready.size.x
...
## add explicit type - Haley
#func parse_glossary(dialog_script):
func parse_glossary(dialog_script: Dictionary) -> Dictionary:
## end override
	var words = []
	for g in glossary:
	...
	## Handle the case where dialog_script.events does not exist
	# (EVEN THOUGH IT REALLY FUCKING SHOULD),
	## which fails on export, apparently - Haley
	#if words.size() > 0:
	if words.size() and "events" in dialog_script:
		var index = 0
...
func _process(_delta):
	## override default next indicator behavior
	## and implement texture change of texturerect -Haley
	#$TextBubble/NextIndicator.visible = finished
	if is_instance_valid($TextBubble/TextureRect) and $TextBubble/TextureRect is NinePatchRect:
		($TextBubble/TextureRect as NinePatchRect).set_region_rect(rect_ready if finished else rect_notready)
	## add beepboops
	if characters_tweening:
		var characters_shown_now = $TextBubble/RichTextLabel.visible_characters
		if characters_shown_now > 0 and characters_shown_now > characters_shown:
			## if the newly shown character isn't whitespace, request a beepboop
			if !(
				$TextBubble/RichTextLabel.get_text().ord_at(characters_shown_now - 1)
			in [
				" ".ord_at(0), # space
				"　".ord_at(0) # "fullwidth" space
			]):
				Root.request_sfx_random("dialog")
			characters_shown = characters_shown_now
	## end override -Haley
	if Engine.is_editor_hint() == false:
		# Multiple choices
...
func start_text_tween():
	...
	## I want beepboops - Haley
	characters_tweening = true
	characters_shown = -1
	## end override - Haley
...
func update_name(character, color='#FFFFFF'):
	if character.has('name'):
		...
		## Add translation support - Haley
		parsed_name = TranslationServer.translate(parsed_name)
		## end override
...
func update_text(text):
	...
	$TextBubble/RichTextLabel.bbcode_text = TranslationServer.translate(text)
	## end override
...
func add_choice_button(option):
	...
	## We're just gonna comment this whole style thing out
	## because we're using our Theme instead
	## -Haley
#	# Text
...
#	button.get_node('TextureRect').set('margin_bottom', settings['button_offset_y'])
	...
	## Focus the first choice button for non-mouse purposes - Haley
...
func load_theme() -> void:
	...
	#	text_speed = settings['theme_text_speed'] * 0.01
		text_speed = settings['theme_text_speed'] * (1.0 / 60.0)
	## end override - Haley
	# Margin
