...
static func get_path(name: String, extra: String ='') -> String:
	...
	if extra != '':
		if name in paths:
			## override:
			# Somehow there's an extra slash at the start sometimes. - Haley
			if paths[name].ends_with("/") or extra.begins_with("/"):
				# one of them has a slash
				return paths[name] + extra
			elif paths[name].ends_with("/") and extra.begins_with("/"):
				# both of them have a slash
				return paths[name] + extra.substr(1)
			## end override
			return paths[name] + '/' + extra
		return ""
	else:
...
static func listdir(path: String) -> Array:
	...
	var RESULT000 = dir.open(path)
	var RESULT001 = dir.list_dir_begin()
	while true:
